...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gubet is "gubet". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gubet", keep that.
	if name == "gubet" || name == "gubet-testnet" {
		name = "Gubet"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gubet" instances.
var isOldGubetResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gubet 1.4 are used if they exist.
	if warn, isOld := isOldGubetResource[path]; isOld {
		oldpath := ""
		if c.name() == "gubet" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGubetResourceWarning, "Using deprecated resource file %s, please move this file to the 'gubet' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
